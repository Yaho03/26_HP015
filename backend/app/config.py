...
class Settings(BaseSettings):
    ...
    #
    # ⚠ 근거 미확보. 05_DIGITAL_TWIN_SPEC §5.1 의 0.5m 는 축소 데모 공간 기준이고,
    # 60m 화물창(ship-visual)에 그대로 쓰면 점이 된다. 센서는 y=±3.25 에 있고
    # 통행로는 y=0 이라 0.5m 로는 **어떤 경보가 떠도 경로에 영향을 주지 못한다** —
    # FR-803 이 죽은 코드가 된다.
    # 기본값 4.0 은 센서선(±3.25)에서 중앙 통로(y=0)까지 닿고 약간의 여유를 둔
    # 값이며, 실측이나 확산 모델 근거는 없다. 가스 확산 반경 산정은 미결이다.
