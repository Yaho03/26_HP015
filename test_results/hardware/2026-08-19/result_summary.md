...
**판정: [x] 실물 통과 (단, 실물은 1개 노드 한정)** — 4노드 동시 부하는 8/20 주입 시나리오로 확인했다 (§2.9).
...
### 2.9 8/19 세션에서 못 한 항목 — 후속 검증 결과

8/19 에는 보드 가용 대수와 시험 조건이 부족해 아래 6건을 넘겼다. 그 뒤 소프트웨어 경로로 5건을 확인했다.

| 항목 | 8/19 상태 | 후속 검증 |
|---|---|---|
| #54 시간 기반 판정 (CO2 1000ppm 3초) | CO2 불기 시험 못 함 | **확인 (8/20)** — 1,100ppm 주입 06:24:26 시작 → 3초 유지 후 06:24:29 `level1_caution` 발화 |
| #55 Hysteresis / De-escalation | 못 함 | **확인 (8/20)** — `level3_critical` → `level2_warning` → `level1_caution` → `normal` 순차 해제. exit 임계값 4500 / 1900 / 900 이 각각 적용됨 |
| #49 4노드 동시 부하 | 노드 1개만 가용 | **확인 (8/20)** — sensor-01~04 동시 주입, 노드당 분당 30행 정상 적재 |
| #13 #162 추세 기반 선제 표시 | 못 함 | **확인 (8/20)** — 대시보드 노드 카드에 `추세 유지 시 약 N분 뒤 L1 주의 도달 예상` 표시 |
| #159 #160 트윈 좌표·판독성 | 못 함 | **확인 (#164 작업)** — `worker_walk` 주입 후 평면도 마커 SVG 좌표를 읽어 Z-up→Y-up 부호 일치 확인. IDW 캡션(FR-402 MUST) 표시 확인 |
| #67 E2E 경보 지연 (화면 렌더까지) | 부분만 — 발행 지연 5.1ms 측정 | 미완 — 화면 렌더까지의 지연은 아직 측정 안 함 |

> **후속 검증은 MQTT 주입 시나리오로 한 것이라 소프트웨어 경로 판정이다.** 실물 센서로 같은 항목을 다시 하려면 8/19 조건(보드 4대 동시 가용)이 필요하다.
