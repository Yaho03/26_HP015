...
| EXP-8.1 | 최단 경로 위에 `level3` 구역 주입 | 우회 경로로 전환, 트리거 → 발행 지연 ≤ 1초 (P95) | ⚠ 경로 전환은 자동화 완료, **지연은 미측정** |
...
자동화되지 않은 유일한 항목이다. WebSocket 배선이 끝났으므로 다음 방식으로 실측한다.

1. 대시보드를 열고 `evacuation_route` 메시지의 `computed_at`을 기록한다.
2. 주입 도구로 최단 경로 위 센서에 `level3` 농도를 발행한다.
3. 발행 시각 → 클라이언트 수신 시각의 차이를 30회 측정해 P95를 낸다.
