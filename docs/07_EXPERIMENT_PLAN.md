...
| EXP-7.1 | 적산 중 백엔드 강제 재시작 | 복구 후 dose 연속성 유지, 손실 ≤ `flush_interval_s` 분량 | ⚠️ **미검증** — 아래 참조 |
...
### EXP-7.1 이 왜 미검증인가 (정직하게)

복구 경로 자체는 구현돼 있고 기동 시 실제로 호출된다 —
`exposure_service._recover()`가 `load_active_states()`로 열린 윈도우를 메모리로 되살리고,
`app/main.py`의 lifespan이 이를 호출한다.

**그런데 그 경로를 검증하는 테스트가 없다.** 서비스 테스트는 `load_active_states`를 가짜로
바꿔 놓고 돌기 때문에, 실제 DB에 적산 중이던 행이 남았을 때 재기동 후 dose가 이어지는지는
확인된 적이 없다. 종료 flush(`stop()`)와 주기 flush는 각각 테스트가 있으나,
**"flush → 프로세스 종료 → 재기동 → 연속"** 을 한 줄로 꿴 검증은 아직 없다.

측정하려면 통합 스택에서 다음이 필요하다.

1. 작업자를 배정하고 알려진 농도를 일정 시간 주입해 dose를 쌓는다
2. `flush_interval_s`가 한 번 지난 뒤 백엔드 컨테이너만 재시작한다
3. 재기동 후 `GET /api/exposure/current`의 `dose_ppm_min`이 종료 직전 값에서
   **이어지는지**, 손실이 `flush_interval_s` 분량 이내인지 본다

**결과 보고 시 이 항목을 "통과"로 적어서는 안 된다.** 미실시다.
