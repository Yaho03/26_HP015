...
축소 데모 공간 좌표를 선박형 3D 트윈에 표시하는 경우에는 먼저
`demo-local` → `ship-visual` 비율 매핑을 적용하고, 그 결과 좌표에 아래의
Z-up → Y-up 축 변환을 적용한다.

현재 선박형 표시 공간(`ship-visual`)의 기준 크기는 길이 60m x 폭 20m x 높이 14m이다.
이 문서의 2.0m x 1.5m 센서 배치와 예제 좌표는 축소 데모(`demo-local`) 예시로만
해석해야 하며, 실제 표시 공간의 크기와 혼동하지 않는다. 실제값은 원본 좌표로
보존하고, 화면용 좌표는 별도 매핑 결과로 생성한다.
