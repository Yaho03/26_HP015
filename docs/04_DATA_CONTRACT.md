...
> **백엔드 재시작 시 경보 상태 복구**: 백엔드는 시작 시 `alert_events`에서 `(source_node_id, alert_key)`별 최신 행을 읽고, 최신 상태가 `active`인 경보를 발행기와 `AlertEvaluator`에 함께 복구한다. DB를 복구의 단일 원천으로 사용하며 `alerts/state/#` Retain 메시지를 백엔드가 다시 구독하지 않는다. 복구된 level은 이미 active인 상태로 유지하고 `enter_for_ms` 타이머를 다시 시작하지 않는다. 최초 `activated_at`은 발행기 추적 상태에 그대로 보존한다. 복구 실패는 로그로 드러내되 신규 경보 수집까지 막지 않도록 서버 기동은 계속한다 (#194, #196).
