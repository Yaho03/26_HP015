...
사양서 §3.2 가 요구한 P0-A 는 **사람이 고시 원문을 펼쳐 대조하는 절차**다.

### 남아 있는 근거

PR [#201](https://github.com/Yaho03/26_HP015/pull/201) 본문에 이렇게 적혀 있다.

> 고용노동부고시 제2020-48호(2020-01-14) 별표 1의 영문명·화학식·CAS·TWA/STEL ppm 열을
> **원문 HWP에서 대조했다.**

대조 대상(별표 1)과 대조한 열(영문명·화학식·CAS·TWA/STEL)까지 특정돼 있어, 기억으로
적은 문장의 모양은 아니다. 다만 **누가 그 HWP 를 열었는지는 어디에도 없다.**

### 비어 있는 것 — 서명

시드 커밋(`88c1e72`)은 한 줄짜리 메시지라 본문이 없고, PR 본문의 문장은 작성 주체가
명시돼 있지 않다. 아래 세 줄이 채워지기 전까지 이 시드는 "확인됨"이 아니다.

```
확인자:
확인일:
확인한 원문 형태:   (국가법령정보센터 PDF / 고용노동부 고시 HWP / 인쇄본 중 하나)
```

**AI 가 이 세 줄을 채우지 않는다.** 숫자가 그럴듯하다는 판단은 원문 대조가 아니고,
그 구분이 P0-A 규칙의 전부다. 위 인용문을 쓴 주체가 사람이면 그 사람이 서명하면 끝난다.
