...
`traverse_factor` 기본값 (설정 파일에서 재정의 가능). 계수는 **경로 간 비교를
위한 상대값**이다 — 어느 경로가 더 나은지 고르는 척도이지, 대피 소요 시간의
예측이 아니다 (절대 시간은 `walk_speed_mps` 기준으로 따로 나눈다).

유도 근거 (OQ-V3, 2026-08-22 문헌 확보): IMO MSC.1/Circ.1533 Rev.1 §1.3
Table 1.3 의 무장애 이동 속도 — **복도 1.2 m/s, 계단 하강 1.0 m/s, 계단 상승
0.8 m/s** — 를 닻으로 삼아 복도 대비 비율로 정했다.

| `kind` | 기본값 | 근거 (복도 1.2 m/s = 1.0 기준) |
|--------|-------:|------|
| `walk` | 1.0 | 기준. IMO 복도 1.2 m/s |
| `scaffold_plank` | 1.3 | 좁고 미끄러운 발판 — IMO 계단 하강(1.2배) 상당에서 미끄럼 여유 |
| `hatch` | 1.8 | 해치 통과는 자세 변경·기어오름 — 계단 상승(1.5배)과 사다리(2.5) 사이 |
| `ladder` | 2.5 | 수직·양손 사용 — 계단 상승(1.5배)을 넘고, NIST TN 1839 혼잡 계단 하강 평균 0.44 m/s(≈2.7배)의 하한 쪽 |

| 참고 매개변수 | 값 | 근거 |
|---|---:|------|
| `walk_speed_mps` (절대 시간 환산 기준) | 0.8 m/s | IMO 계단 하강 무장애 1.0 m/s 에서 PPE·밀폐공간 저시야 보수 반영 |

한계: 위 문헌은 일반 대피(승객·사무실 인구) 기반이다. **화물창 작업복·장비
착용 상태의 실측이 아니므로** 실측 확보 시 재조정 대상이고, `estimated_seconds`
는 참고 정보로만 제시한다 (§7 면책 참조).
...
| OQ-V3 | `traverse_factor` 기본값의 근거 문헌 | **근거 확보** — IMO MSC.1/Circ.1533·NIST TN 1839 파생 (§3.2). 밀폐공간 PPE 실측은 아님 — 실측 시 재조정 |
