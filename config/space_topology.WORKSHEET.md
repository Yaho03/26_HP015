...
> **이미 실측 기반인 부분**: `nav.zone.*` 6개 노드(+링 엣지 e009~e016)는 UWB
> 앵커 실측 공간(2.5 x 2.0m)을 균일 배율 6.5 로 매핑한 스트립에서 유도됐다
> (2026-08-22, 이슈 #225). 이 노드들이 실측 작업자 위치의 스냅 커버리지를
> 담당한다 — `backend/tests/test_topology_coverage.py` 가 불변식을 잠근다.
> 현장 측정 시 구역 노드의 demo-local 원본 좌표(x 0.33/1.25/2.17m,
> y 0.42/1.58m)를 실측값으로 보정하면 된다.
